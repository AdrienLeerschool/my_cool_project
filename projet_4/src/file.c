...
/*static void merkle_tree(struct kiocb* iocb, ssize_t written_bytes)
{
	struct super_block *sb = file ->f_inode->i_sb;
	int block_size = EXT4_BLOCK_SIZE(sb);
	int merkle_size = block_size / 2; 
	// minus one because only half a block
	// minus 4 because each hash will be 16 bytes long
	int max_depth = EXT4_MIN_BLOCK_LOG_SIZE + sb -> s_log_block_size - 1 - 4;
	int number_leaves = 1 << max_depth;
	struct file* filp = iocb -> ki_filp;
	loff_t start_pos = filp -> f_pos;
	struct inode* inode = filp -> f_inode;
	//+1 to account for truncation
	long size_datablock = inode -> i_size / number_leaves + 1;
	char hash_tree_str[12]  = "trusted.merkle";
	char size_datablock_str[20] = "trusted.size_datablock";
	char buf[33];
	char *prev_merkle = kmalloc(merkle_size, GFP_KERNEL);
	long prev_size_datablock;
	inode -> i_op -> getxattr(filp -> f_path.dentry, 	
						 	  size_datablock_str, buf, 32);
	buf[32] = '\0';
	kstrtol(buf, 0, &prev_size_datablock);
	inode -> i_op -> getxattr(file -> f_path.dentry, 
							  hash_tree_str, prev_merkle, merkle_size);
	if(prev_size_datablock != size_datablock)
	{
		init_merkle_tree(inode, filp);
		goto out;
	}
	else
		// minus one because it's an index
		filp -> f_pos = (filp -> f_pos / size_datablock) * (size_datablock - 1);

	inode -> i_op -> setxattr(file -> f_path.dentry, 
							  name, (void*)value, merkle_size, 0);
	out:
...
static ssize_t my_write(struct kiocb *iocb, struct iov_iter *from)
{
	printk(KERN_ALERT "my write entry\n");
	struct file *filp = iocb->ki_filp;
	struct inode *inode = filp->f_inode;
    struct dentry *dentry = filp->f_path.dentry;
	ssize_t ret = ext42_file_write_iter(iocb, from);
	if(ret >= 0 
	   && inode->i_op->getxattr(dentry, "user.mt", NULL, 0) < 0)
		update_merkle_tree(iocb, ret);
	printk(KERN_ALERT "my_write exit\n");
	return ret;
}
